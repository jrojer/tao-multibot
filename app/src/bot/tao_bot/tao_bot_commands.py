...
from app.src.bot.handlers.help_handler import get_help_command_handler
from app.src.bot.handlers.setters import (
    get_set_number_of_messages_per_completion_handler,
)
...
def _command(bot_username: str, cmd: str) -> str:
    return "/" + cmd + "@" + bot_username


def _extract_cmd(post: str) -> str:
...
class TaoBotCommands:
    ...
    def _commands(self):
        return {
            ...
            CONFIG: get_help_command_handler(self._tao_bot_conf, self._gpt_conf),
            SET_NUMBER_OF_MESSAGES_PER_COMPLETION: get_set_number_of_messages_per_completion_handler(
                self._client
            ),
