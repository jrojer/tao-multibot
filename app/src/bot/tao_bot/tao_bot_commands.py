...
from app.src.bot.handlers.setters import (
    get_set_frequency_penalty_handler,
    get_set_max_tokens_handler,
    get_set_number_of_messages_for_completion_handler,
    get_set_presence_penalty_handler,
    get_set_temperature_handler,
    get_set_top_p_handler,
)
...
class TaoBotCommands:
    ...
    def _commands(self):
        return {
            ENABLE_IN_CURRENT_GROUP: get_enable_in_group_handler(self.config),
            DISASBLE_IN_CURRENT_GROUP: get_disable_in_group_handler(self.config),
            UPDATE_ACCESS: get_update_access_handler(self.config),
            START: get_start_handler(),
            CONFIG: get_help_command_hadler(self.config),
            SET_TEMPERATURE: get_set_temperature_handler(self.config),
            SET_TOP_P: get_set_top_p_handler(self.config),
            SET_MAX_TOKENS: get_set_max_tokens_handler(self.config),
            SET_PRESENCE_PENALTY: get_set_presence_penalty_handler(self.config),
            SET_FREQUENCY_PENALTY: get_set_frequency_penalty_handler(self.config),
